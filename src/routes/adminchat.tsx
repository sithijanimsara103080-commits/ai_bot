...
export const Route = createFileRoute("/adminchat")({
  head: () => ({ meta: [{ title: "Admin Console — Mission Astro AI" }] }),
...
function AdminChatPage() {
  ...
  const [principalPopupOpen, setPrincipalPopupOpen] = useState(true);

  // Auth guard completely removed so anyone can enter.
  ...
  const send = useCallback(async (text: string) => {
    ...
    try {
      ...
      // Use a consistent ID for the admin/principal
      const payload: Record<string, any> = {
        message: content || "",
        userId: "admin_principal_user",
        mode: mode.toLowerCase() as "fast" | "super" | "master" | "image",
  ...
  return (
    <div className="h-screen flex text-foreground overflow-hidden" style={{ background: "var(--gradient-space)" }}>
      ...
      <Dialog open={principalPopupOpen} onOpenChange={setPrincipalPopupOpen}>
        <DialogContent className="max-w-md mx-4 bg-card/95 backdrop-blur-xl border-white/20 rounded-2xl p-6 shadow-2xl">
          <div className="text-center py-2 space-y-4">
            <div className="h-16 w-16 mx-auto rounded-full bg-primary/20 flex items-center justify-center shadow-[0_0_20px_rgba(249,115,22,0.3)]">
              <Crown className="h-8 w-8 text-primary" />
            </div>
            <h2 className="text-2xl font-bold tracking-tight text-white drop-shadow-md">
              Welcome, Principal Mr. Kumara Piyathilaka!
            </h2>
            ...
            <div className="pt-4">
              <Button onClick={() => setPrincipalPopupOpen(false)} className="w-full text-base font-semibold h-12 shadow-[0_0_15px_rgba(239,68,68,0.5)]" style={{ background: "linear-gradient(135deg, #f97316, #ef4444)", color: "white" }}>
